...
public class AnalysisHelper {
    ...
    public void getMaxLikeCommentPath(){
       // Map<Integer,Comment> comments= Comments.get
       ...
       System.out.println("Q2- post with most likes per comment : " 
               + data.getPosts().get(postId).getPostId());
    ...
    public void getPostWithMostComments(){
        ...
        for (Post p:data.getPosts().values()){
         ...
         System.out.println("Q3- post with most comments : " 
               + postwithMostComments.getPostId());
    ...
    public void getPassiveUsers(){
       ...
       for(int i=0;i<5;i++){
           System.out.println(users.get(i)+
                   ",- Post count"+postNumbers.get(users.get(i).getId()));
       }
     ...
     public void getPassiveCommentUsers(){
       ...
       for(int i=0;i<5;i++){
           System.out.println(users.get(i)+
                   ",- Post count"+commentNumbers.get(users.get(i).getId()));
       }
     }   
      ...
      public void getPassiveAndActiveoverallUsers(){
        ...
        for(Comment c : data.getComments().values()){
            ...
            overallNumbers.put (userId, overallNumbers.get (userId) + + c. getLikes() );
            }else{
            overallNumbers.put (userId, + c.getLikes()) ;
            }
       ...
       for(int i=0;i<5;i++){
           System.out.println(users.get(i)+
                   ",- Post count"+overallNumbers.get(users.get(i).getId()));
       }
